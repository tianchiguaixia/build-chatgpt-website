...
$ceshiqq = $_POST['ceshiqq'];
$ceshiqq = str_replace("@qq.com", "", $ceshiqq);
...
if($yzmfs == '1'){
    ...
    if (strpos($yxqq, '@qq.com') !== false) {
        
        $mail->From = $yxqq;
 
} else {
       $mail->From = $yxqq."@qq.com";
}
    ...
    //设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”,x代表阿拉伯数字（你的QQ号）
        ...
        if (strpos($qqqq, '@qq.com') !== false) {
        
      $mail->From = $qqqq;
 
} else {
      $mail->From = $qqqq."@qq.com";
}
...
//设置收件人邮箱地址 该方法有两个参数 第一个参数为收件人邮箱地址 第二参数为给该地址设置的昵称 不同的邮箱系统会自动进行处理变动 第二个参数会在发送成功的邮件收件人那边显示
        ...
        if (strpos($ceshiqq, '@qq.com') !== false) {
        
    $mail->addAddress($ceshiqq,'xxx163邮箱');
 
 
} else {
     $mail->addAddress($ceshiqq.'@qq.com','xxx163邮箱');
...
if($sfces == '1'){
    $flag = sendMail($ceshiqq, '测试短信'.'-时间:'.date("Y-m-d H:i:s"), "测试短信");
    if ($flag) {
    echo "发送成功 测试";
} else {
    echo "发送失败 测试";
    ...
    $flag = sendMail($qqqq, '网站注册验证码', "网站注册验证码：$code");
